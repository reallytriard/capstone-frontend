...
type PreviewMode = 'desktop' | 'tablet' | 'mobile' | 'android';
...
const PREVIEW_OPTIONS: Array<{ value: PreviewMode; label: string; widthLabel: string }> = [
  ...
  { value: 'android', label: 'Android', widthLabel: '412px' }
];
      ...
      try {
        ...
        const backendBaseUrl =
          window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1'
            ? 'http://localhost:8888'
            : 'https://nine900-aeew.onrender.com';

        const response = await fetch(`${backendBaseUrl}/import/story_upload`, {
          method: 'POST',
          body: formData
        });
    ...
    >
      ...
      <main className="story-content">
        {sectionsWithGlobalIndex.map((section: any, index: number) => {
          switch (section.type) {
            case 'hero':
              return <HeroBlock key={`hero-${index}`} {...section} />;
            case 'video':
